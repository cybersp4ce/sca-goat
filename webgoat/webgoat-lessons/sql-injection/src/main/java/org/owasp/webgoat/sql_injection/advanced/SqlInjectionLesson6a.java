...
public class SqlInjectionLesson6a extends AssignmentEndpoint {
    ...
    // moved the lined just a bit 
    public AttackResult injectableQuery(String otherAccountName) {
        String query = "";
        try (Connection connection = dataSource.getConnection()) {
            boolean usedUnion = true;
            query = "SELECT * FROM other_user_data WHERE last_name = '" + otherAccountName + "'";
            //Check if Union is used
            if (!otherAccountName.matches("(?i)(^[^-/*;)]*)(\\s*)UNION(.*$)")) {
                usedUnion = false;
            }
