...
public class SqlInjectionLesson6a extends AssignmentEndpoint {
    ...
    public AttackResult injectableQuery(String otherAccountttta) {
        String query = "";
        try (Connection connection = dataSource.getConnection()) {
            boolean usedUnion = true;
            query = "SELECT * FROM other_user_data WHERE last_name = '" + otherAccountttta + "'";
            //Check if Union is used
            if (!otherAccountttta.matches("(?i)(^[^-/*;)]*)(\\s*)UNION(.*$)")) {
                usedUnion = false;
            }
